      ...
      <aside className="w-full lg:w-72 bg-slate-900 text-white p-8 flex flex-col shrink-0">
        <div className="mb-12">
          <h2 className="text-xl font-black tracking-tighter flex items-center gap-2">
            <span className="p-2 bg-orange-500 rounded-xl">⚙️</span>
            إدارة فاقوس
          </h2>
          <p className="text-slate-400 text-[10px] font-bold uppercase mt-1 tracking-widest">نظام الإدارة v2.5</p>
        </div>
        
        <nav className="space-y-2 flex-grow">
          <AdminNavButton to="/admin/products" label="المخزون والمنتجات" icon="📦" badge={stats.outOfStockCount > 0 ? stats.outOfStockCount : undefined} />
          <AdminNavButton to="/admin/stats" label="الإحصائيات العامة" icon="📊" />
          <AdminNavButton to="/admin/categories" label="الأقسام والتصنيفات" icon="🏷️" />
          <AdminNavButton to="/admin/orders" label="طلبات العملاء" icon="🛍️" badge={stats.pendingOrders} />
        </nav>

        <div className="pt-8 border-t border-slate-800 mt-auto">
          <NavLink to="/" className="w-full text-right p-4 rounded-2xl text-slate-400 hover:text-white hover:bg-slate-800 transition block font-black text-sm">
            الرجوع للمتجر 🏪
          </NavLink>
        </div>
      </aside>
      ...
      <main className="flex-grow p-6 lg:p-12 bg-slate-50/50 overflow-y-auto no-scrollbar">
        ...
        <div className="mb-10 flex flex-col md:flex-row md:items-center justify-between gap-6">
          <div className="relative flex-grow max-w-md">
            {activeTab === 'products' ? (
              <input 
                ...
                className="w-full px-6 py-3 bg-white border border-slate-200 rounded-2xl outline-none focus:ring-2 focus:ring-orange-500 font-bold text-sm shadow-sm"
              />
          ...
          <div className="flex gap-3">
            ...
            <button 
                ...
                className="bg-orange-500 text-white px-8 py-3 rounded-2xl font-black shadow-xl shadow-orange-100 hover:scale-105 transition active:scale-95"
            >
                + إضافة محصول جديد
            </button>
          </div>
        ...
        {activeTab === 'stats' && (
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 animate-fadeIn">
            <StatCard title="إجمالي المبيعات" value={`${stats.revenue} ر.س`} icon="💰" color="text-orange-500" />
            <StatCard title="الطلبات الجديدة" value={stats.pendingOrders} icon="🔥" color="text-orange-400" />
              ...
              <tbody className="divide-y divide-slate-50">
                {filteredProducts.map(p => (
                  <tr key={p.id} className="hover:bg-slate-50/50 transition">
                    ...
                    <td className="px-8 py-4 font-black text-orange-500 text-sm">{p.price} ر.س</td>
                    <td className="px-8 py-4">
          ...
          <div className="space-y-12 animate-fadeIn">
            <div className="bg-white p-8 rounded-[2.5rem] border shadow-sm max-w-2xl">
              <div className="flex items-center gap-4 mb-6">
                <span className="p-3 bg-orange-50 text-orange-500 rounded-2xl text-xl">✨</span>
                <div>
              ...
              <div className="flex gap-3">
                <input 
                  ...
                  className="flex-grow px-6 py-4 bg-slate-50 rounded-2xl outline-none focus:ring-2 focus:ring-orange-500 font-bold transition shadow-inner"
                />
                <button 
                  onClick={() => { if(newCatName) { onAddCategory({id: 'cat_'+Date.now(), name: newCatName}); setNewCatName(''); } }}
                  className="bg-slate-900 text-white px-10 rounded-2xl font-black hover:bg-orange-500 transition shadow-lg active:scale-95"
                >
                  إضافة القسم
                </button>
              </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
              {filteredCategories.map(cat => {
                ...
                return (
                  <div key={cat.id} className="bg-white p-8 rounded-[2.5rem] border border-slate-100 shadow-sm flex flex-col gap-6 group hover:border-orange-200 hover:shadow-xl transition-all relative overflow-hidden">
                    <div className="absolute top-0 right-0 w-24 h-24 bg-orange-50 rounded-bl-[5rem] -mr-8 -mt-8 opacity-40 group-hover:bg-orange-100 transition-colors"></div>
                        ...
                        {editingCatId === cat.id ? (
                          <div className="flex items-center gap-2">
                            <input 
                              ...
                              className="bg-white border-2 border-orange-200 px-4 py-2 rounded-xl outline-none font-bold text-sm w-40"
                              autoFocus
                            />
                            <button onClick={() => handleUpdateCategory(cat.id)} className="p-2 bg-orange-500 text-white rounded-xl shadow-md">✓</button>
                            <button onClick={() => setEditingCatId(null)} className="p-2 bg-slate-100 text-slate-400 rounded-xl">✕</button>
                          </div>
                        ) : (
                          <div>
                    ...
                    <div className="flex items-center justify-between mt-auto pt-4 border-t border-slate-50 relative z-10">
                      ...
                      <div className="flex gap-2 opacity-0 group-hover:opacity-100 transition-all transform translate-y-2 group-hover:translate-y-0">
                        <button onClick={() => { setEditingCatId(cat.id); setEditingCatName(cat.name); }} className="p-3 bg-blue-50 text-blue-600 rounded-2xl hover:bg-blue-600 hover:text-white transition shadow-sm">✎</button>
                        <button onClick={() => onDeleteCategory(cat.id)} className="p-3 bg-red-50 text-red-600 rounded-2xl hover:bg-red-600 hover:text-white transition shadow-sm">🗑</button>
                      </div>
                    </div>
                  </div>
                );
        ...
        {activeTab === 'orders' && (
          <div className="bg-white rounded-[2.5rem] shadow-sm border border-slate-100 overflow-hidden animate-fadeIn">
            {orders.length > 0 ? (
                ...
                <tbody className="divide-y divide-slate-50">
                  {orders.map(o => (
                    <tr key={o.id} className="hover:bg-slate-50/50 transition">
                      ...
                      <td className="px-8 py-4 font-black text-orange-500 text-sm">{o.total} ر.س</td>
              ...
              <div className="p-20 text-center text-slate-400 font-bold">لا توجد طلبات مسجلة حالياً</div>
            )}
          </div>
        )}
      </main>
...
const AdminNavButton = ({ to, label, icon, badge }: any) => (
  <NavLink 
    to={to} 
    className={({isActive}) => `w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-black text-sm transition-all ${
      isActive ? 'bg-orange-500 text-white shadow-xl shadow-orange-900/10' : 'text-slate-400 hover:bg-slate-800 hover:text-white'
    }`}
  >
