...
const CartView: React.FC<CartViewProps> = ({ cart, onUpdateQuantity, onRemove, onCheckout, onContinueShopping }) => {
  ...
  const handleManualQuantityChange = (id: string, value: string) => {
    ...
    if (!isNaN(num) && num >= 0) {
      // إرسال الفرق المطلوب للوصول للقيمة الجديدة
      const currentItem = safeCart.find(i => i.id === id);
      if (currentItem) {
        onUpdateQuantity(id, num - currentItem.quantity);
      }
    }
  };
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        <div className="space-y-3 md:space-y-4">
          {safeCart.map((item, idx) => {
            ...
            const isWeightUnit = item.unit === 'kg' || item.unit === 'gram';
              ...
              <div key={`${item.id}-${idx}`} className="bg-white p-3 md:p-5 rounded-[1.5rem] md:rounded-[2rem] flex gap-3 md:gap-6 items-start border border-gray-100 hover:shadow-lg transition-all group relative overflow-hidden">
                ...
                <div className="flex-grow flex flex-col min-w-0 h-full justify-between py-0.5">
                  <div className="space-y-1">
                    ...
                    <span className="text-[9px] font-black text-slate-400">الوحدة: {item.unit === 'kg' ? 'كيلو جرام' : item.unit === 'gram' ? 'جرام' : 'قطعة'}</span>
                  </div>

                  <div className="flex items-center justify-between mt-3 md:mt-4">
                    {/* التحكم في الكمية - يدعم الكسور */}
                    <div className="flex items-center bg-slate-50 rounded-lg md:rounded-xl border border-slate-100 p-1">
                      <button 
                        onClick={() => onUpdateQuantity(item.id, isWeightUnit ? 0.1 : 1)}
                      ...
                      <input 
                        ...
                        className="bg-transparent border-none outline-none font-black text-slate-700 text-xs md:text-base w-12 md:w-16 text-center focus:ring-0"
                      />
                    ...
                    <div className="flex flex-col items-end gap-1">
                       <div className="text-sm md:text-xl font-black text-indigo-600">
                        {(item.price * item.quantity).toFixed(2).toLocaleString()} <small className="text-[8px] md:text-[10px] font-bold">ج.م</small>
                      </div>
                      <button 
