<?php
/**
 * API Backend for Elite Store - Robust Edition
 */

// عرض الأخطاء أثناء التطوير (يمكن إغلاقها في الإنتاج)
...
try {
    ...
    if (!file_exists('config.php')) {
        throw new Exception("ملف الإعدادات config.php غير موجود.");
    }
    require_once 'config.php';

    // التأكد من وجود المتغير $pdo
    if (!isset($pdo)) {
        throw new Exception("فشل تعريف متغير الاتصال بقاعدة البيانات.");
    }

    // تهيئة الجداول تلقائياً إذا كانت مفقودة
    ...
    switch ($action) {
        ...
        case 'add_product':
            ...
            if (!$d) throw new Exception("بيانات المنتج غير صالحة.");
        ...
        case 'update_product':
            $d = json_decode(file_get_contents('php://input'), true);
            if (!$d) throw new Exception("بيانات التحديث غير صالحة.");
        ...
        case 'add_category':
            $d = json_decode(file_get_contents('php://input'), true);
            if (!$d) throw new Exception("بيانات القسم غير صالحة.");
        ...
        case 'save_order':
            $d = json_decode(file_get_contents('php://input'), true);
            if (!$d) throw new Exception("بيانات الطلب غير صالحة.");
