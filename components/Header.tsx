  ...
  return (
    ...
    >
      <div className="container mx-auto px-2 md:px-4">
        <div className={`bg-white border border-slate-100 rounded-[1.5rem] md:rounded-[2rem] px-3 md:px-6 py-2 md:py-3 flex items-center justify-between gap-3 md:gap-8 shadow-lg transition-all duration-500 ${scrolled ? 'mx-1 md:mx-2 shadow-indigo-100/20' : 'mx-0 shadow-slate-200/50'}`}>
...
const ActionButton = ({ count, icon, onClick, variant = 'secondary', className = '' }: any) => (
  <button 
    onClick={onClick}
    className={`relative p-2 md:p-3 rounded-xl md:rounded-2xl transition-all active:scale-90 ${variant === 'primary' ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-slate-50 text-slate-600 hover:bg-white border border-slate-100'} ${className}`}
  >
