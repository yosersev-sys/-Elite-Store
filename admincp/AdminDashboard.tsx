...
const AdminDashboard: React.FC<AdminDashboardProps> = ({ 
  ...
  // إعدادات الترقيم (Pagination) للطلبات
  ...
  // منطق تقسيم الطلبات (Pagination Logic)
  const paginatedOrders = useMemo(() => {
    const startIndex = (orderPage - 1) * ordersPerPage;
    return filteredOrdersTable.slice(startIndex, startIndex + ordersPerPage);
                  ...
                  {paginatedOrders.length > 0 ? (
                    paginatedOrders.map(order => (
                      <tr key={order.id} className={`hover:bg-slate-50/50 ${order.status === 'cancelled' ? 'opacity-50 grayscale' : ''}`}>
                        ...
                        <td className="px-8 py-5 font-black text-emerald-600">{(order.total || 0).toLocaleString()} ج.م</td>
                        <td className="px-8 py-5">
                           <select 
                    ...
                    <tr><td colSpan={6} className="py-20 text-center text-slate-300 font-black italic text-lg">لا توجد طلبات لعرضها</td></tr>
                  )}
                </tbody>
              </table>
            </div>
