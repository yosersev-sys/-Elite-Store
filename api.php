<?php
/**
 * API Backend for Souq Al-Asr - Full Version v4.9
 */
session_start();
error_reporting(0); 
...
function ensureSchema($pdo) {
    // نتحقق من وجود الجداول بدلاً من الاعتماد على الجلسة فقط لضمان الاستقرار
    try {
        ...
        // إذا فشل إنشاء الجداول، لا نوقف السكربت كلياً بل نسجل الخطأ
...
try {
    ...
    switch ($action) {
        ...
        case 'update_order_payment':
            ...
            // التحقق من الكلمات المفتاحية بالإنجليزية أو العربية لتوحيد المدخلات
            if (strpos($method, 'آجل') !== false || strpos($method, 'اجل') !== false || strtolower($method) === 'debt') {
        ...
        case 'add_product':
            ...
            $stmt->execute([$input['id'], $input['name'], $input['description'], (float)$input['price'], (float)$input['wholesalePrice'], $input['categoryId'], $input['supplierId'] ?? null, json_encode($input['images']), (int)$input['stockQuantity'], $input['unit'], $input['barcode'], $input['createdAt'], json_encode($input['seoSettings']), json_encode($input['batches'])]);
            sendRes(['status' => 'success']);
            break;
        ...
        case 'update_product':
            ...
            $stmt->execute([$input['name'], $input['description'], (float)$input['price'], (float)$input['wholesalePrice'], $input['categoryId'], $input['supplierId'] ?? null, json_encode($input['images']), (int)$input['stockQuantity'], $input['unit'], $input['barcode'], json_encode($input['seoSettings']), json_encode($input['batches']), $input['id']]);
            sendRes(['status' => 'success']);
            break;
        ...
        case 'get_current_user': 
            sendRes($_SESSION['user'] ?? ["id" => null, "error" => "No active session"]); 
            break;
