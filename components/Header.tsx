...
const Header: React.FC<HeaderProps> = ({ 
  ...
  return (
    <header className="bg-white shadow-sm z-40 border-b border-gray-100 relative">
      <div className="container mx-auto px-4 py-4">
        <div className="flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              {wishlistCount > 0 && (
                <span className="absolute -top-1 -right-1 bg-red-600 text-white text-[10px] font-black rounded-lg h-5 min-w-[20px] px-1 flex items-center justify-center border-2 border-white shadow-sm">
                  {wishlistCount}
                </span>
              )}
            </button>
        ...
        {/* تصنيفات الهيدر - Chips Navigation */}
        <div className="mt-4 flex items-center gap-3 overflow-x-auto no-scrollbar pb-1">
          <button
            onClick={() => onCategorySelect('all')}
            className={`whitespace-nowrap px-5 py-2 rounded-full text-xs font-black transition-all shadow-sm border ${
      ...
      {isMenuOpen && (
        <div className="lg:hidden border-t border-gray-100 bg-white p-4 space-y-2 animate-fadeIn">
          <button onClick={() => { onNavigate('store'); setIsMenuOpen(false); }} className="w-full text-right p-3 rounded-xl font-bold text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition">الرئيسية</button>
          <button onClick={() => { onNavigate('wishlist'); setIsMenuOpen(false); }} className="w-full text-right p-3 rounded-xl font-bold text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition">المفضلة ({wishlistCount})</button>
          <button onClick={() => { onNavigate('admin'); setIsMenuOpen(false); }} className="w-full text-right p-3 rounded-xl font-bold text-gray-700 hover:bg-indigo-50 hover:text-indigo-600 transition">لوحة الإدارة</button>
          <div className="p-3">
             <input 
              type="text" 
              placeholder="ابحث..." 
