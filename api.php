 ...
 * نظام الإدارة المطور v6.6 - ميزة استرداد الفواتير الذكية
 */
session_start();
...
try {
    ...
    switch ($action) {
        ...
        case 'return_order':
            ...
            $pdo->beginTransaction();
            try {
                // 1. جلب بيانات الطلب للتأكد من وجوده وحالته
                ...
                // 2. إعادة الكميات للمخزن وخصم المبيعات
                ...
                // 3. تحديث حالة الطلب إلى ملغي/مسترد
                $updateOrder = $pdo->prepare("UPDATE orders SET status = 'cancelled' WHERE id = ?");
                $updateOrder->execute([$orderId]);

                $pdo->commit();
                sendRes(['status' => 'success', 'message' => 'تم استرداد الفاتورة بنجاح وإعادة المنتجات للمخزن']);
            } catch (Exception $e) {
                $pdo->rollBack();
