...
function ensureSchema($pdo) {
    // استخدام مجلد النظام المؤقت لملف القفل لتجنب مشاكل صلاحيات المجلدات في الاستضافة
    $cacheFile = sys_get_temp_dir() . '/souq_schema_' . md5(DB_NAME) . '.lock';
    ...
    try {
        ...
        // تحديث الأعمدة المفقودة بهدوء
        $updates = [
            'products' => ['wholesalePrice' => "DECIMAL(10,2) DEFAULT 0", 'unit' => "VARCHAR(20) DEFAULT 'piece'", 'barcode' => "VARCHAR(100)"],
            'orders' => ['city' => "VARCHAR(100) DEFAULT 'فاقوس'", 'status' => "VARCHAR(50) DEFAULT 'completed'"],
            'categories' => ['isActive' => "TINYINT(1) DEFAULT 1", 'image' => "LONGTEXT"]
        ];
        ...
        // لا تعطل الـ API في حال فشل فحص الجداول
    }
}
